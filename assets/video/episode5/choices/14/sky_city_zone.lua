...
while in_interactive_zone do
    ...
    if not sky_city_1_explored then
        ...
        intraFont.print(417 - intraFont.textW(font, choices_fifth.garden, 0.4) / 2 + 8, 145 + 14, choices_fifth.garden, Color.new(255,255,255), font, 0.4)
    end
    ...
    if sky_city_1_explored and not sky_city_2_explored and not castle_guard_talk then
        ...
        intraFont.print(100 - intraFont.textW(font, choices_fifth.castle_guard, 0.4) / 2 + 8, 70 + 14, choices_fifth.castle_guard, Color.new(255,255,255), font, 0.4)
    end
    if not sky_city_2_explored and sky_city_1_explored then
        Image.draw(spritesheet, 417, 60, 15, 15, nil, 430, 0, 15, 15)
        intraFont.print(417 - intraFont.textW(font, "Build site", 0.4) / 2 + 8, 60 + 14, choices_fifth.build_site, Color.new(255,255,255), font, 0.4)
    end

    if sky_city_2_explored and not townspeople_talk then
        Image.draw(spritesheet, 330, 140, 15, 15, nil, 430, 0, 15, 15)
        intraFont.print(330 - intraFont.textW(font, choices_fifth.townspeople, 0.4) / 2 + 8, 140 + 14, choices_fifth.townspeople, Color.new(255,255,255), font, 0.4)
    end

    if sky_city_2_explored then
        Image.draw(spritesheet, 115, 145, 15, 15, nil, 399, 0, 15, 15)
        intraFont.print(115 - intraFont.textW(font, choices_fifth.innkeeper, 0.4) / 2 + 8, 145 + 14, choices_fifth.innkeeper, Color.new(255,255,255), font, 0.4)
    end
